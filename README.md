...
tdx2db 永远免费开源，日线 + 分时 + 复权 + 板块足够跑基本的量化研究。如果你确实需要**逐笔成交**、**高频历史数据**和更多配套维度，可以考虑付费订阅 [Trade Data Plus（tdp）](https://tdp-site.pages.dev/)，数据更全：

- **逐笔成交**：全市场 3 秒级逐笔，带主动买卖方向判定
- **非聚合 1min / 5min**：原始档，不是二次聚合版
- **板块 / 指数成分 + 股票信息**：做轮动、成分回测的配套维度
- **历史包**：一次性导入逐笔、分钟线等高频历史数据
- **跨平台支持**：Linux / macOS / Windows 都可使用，安装和同步流程更完善

TDP 付费订阅会确保数据与客户端向后兼容，并优先处理订阅用户的需求和新增功能。订阅 ¥298/年起，细节见[网站](https://tdp-site.pages.dev/)。
