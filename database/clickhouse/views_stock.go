...
func (d *ClickHouseDriver) registerStockViews() {
...
func (d *ClickHouseDriver) buildStockBFQView() error {
	query := fmt.Sprintf(`
...
func (d *ClickHouseDriver) buildStockHFQView() error {
	query := fmt.Sprintf(`
		CREATE OR REPLACE VIEW %s AS
		SELECT
			s.symbol AS symbol,
...
func (d *ClickHouseDriver) buildStockQFQView() error {
	query := fmt.Sprintf(`
		CREATE OR REPLACE VIEW %s AS
		WITH latest_factors AS (
			SELECT symbol, argMax(hfq_factor, date) AS latest_hfq
