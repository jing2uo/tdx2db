...
func (d *DuckDBDriver) registerViews() {
	...
	d.viewImpls[model.ViewXdxr] = func() error {
		query := fmt.Sprintf(`
			...
			SELECT
				date, code, c1 as fenhong, c2 as peigujia, c3 as songzhuangu, c4 as peigu
	...
	d.viewImpls[model.ViewTurnover] = func() error {
		query := fmt.Sprintf(`
			...
			WITH base_capital AS (
				SELECT date, code, c3 AS float_shares, c4 AS total_shares
				FROM %s WHERE category IN (2, 3, 5, 7, 8, 9, 10)
			),
			expanded AS (
				SELECT d.date, d.symbol,
					LAST_VALUE(c.float_shares IGNORE NULLS) OVER (PARTITION BY d.symbol ORDER BY d.date) AS float_shares,
					LAST_VALUE(c.total_shares IGNORE NULLS) OVER (PARTITION BY d.symbol ORDER BY d.date) AS total_shares
				FROM %s d
				LEFT JOIN base_capital c ON c.code = SUBSTR(d.symbol, 3) AND c.date = d.date
			)
			SELECT
				r.date, r.symbol,
				CASE WHEN e.float_shares > 0 THEN ROUND(r.volume / (e.float_shares * 10000), 6) ELSE 0 END AS turnover,
		...
		`, model.ViewTurnover, model.TableGbbq.TableName, model.TableStocksDaily.TableName, model.TableStocksDaily.TableName)
		_, err := d.db.Exec(query)
		return err
	}
	...
	createAdjustView := func(viewName model.ViewID, sourceTable string, factorCol string, isMin bool) error {
		...
		// 1. 根据是否为分钟线，差异化 SQL 片段
		timeCol := "date"
		dateJoinCond := "s.date = f.date"
		extraSelect := ", t.turnover, t.circ_mv, t.total_mv"
		extraJoin := fmt.Sprintf("LEFT JOIN %s t ON s.symbol = t.symbol AND s.date = t.date", model.ViewTurnover)

		if isMin {
			timeCol = "datetime"
			// DuckDB 中将 timestamp 转为 date 进行关联
			dateJoinCond = "CAST(s.datetime AS DATE) = f.date"
			extraSelect = "" // 分钟线没有计算换手率和市值
			extraJoin = ""
		}
