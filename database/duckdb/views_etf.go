...
func (d *DuckDBDriver) registerETFViews() {
...
func (d *DuckDBDriver) buildETFBFQView() error {
	query := fmt.Sprintf(`
...
func (d *DuckDBDriver) buildETFHFQView() error {
	query := fmt.Sprintf(`
		CREATE OR REPLACE VIEW %s AS
		SELECT
			s.symbol AS symbol,
...
func (d *DuckDBDriver) buildETFQFQView() error {
	query := fmt.Sprintf(`
		CREATE OR REPLACE VIEW %s AS
		WITH latest_factors AS (
			SELECT symbol, argMax(hfq_factor, date) AS latest_hfq
